...
public class AttackActorCollision : MonoBehaviour
{
    ...
    void Update()
    {
        if(enemyHit != null && giveDamage == false)
        {
            ...
            Items itms = Equipment.Instance.GetCurrentSelectedItems();
            ...
            if(itms is Melee m)
            {
                enemyKnock.GetKockedBackInfo(currentlyKnocking, knockBackDirection, m.knockBackLength, m.knockBackPower);
                enemyStunned.GetStunnedInfo(currentlyStunning, m.stunLength);
                enemyHealth.DecreaseHealthWith(m.damage);
            }

            itms.DecreaseDurability();
